  ...
  case Let(x, App(f, ae), e) =>
    ...
    nd[Storable](closures, Ans(Set[VS](), new_cache), { case (cls, clscache, clsnd) =>
      ...
      aeval(body, new_cenv, new_store, new_time, clscache, { case Ans(bdvss, bdcache) =>
        nd[VS](bdvss, Ans(Set[VS](), bdcache), { case (vs, bdvsscache, bdnd) =>
...
Now, what we have obtained is an abstract interpreter written in continuation-passing style.
Though the top-level function @aeval@ has only one continuation, but internally it uses
a @nd@ operator which is written with two layers of continuations. Furthermore, we added
a cache along with the interpreter and @nd@ operator. 
...
The second operator @choices@ is implemented with the delimited control operator 
...
call of @aeval@ to set the delimiter. The afterward updating on cache becomes sequential.
