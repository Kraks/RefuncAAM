...
\begin{center}
 \begin{tikzpicture}
   ...
   }
   {
     ...
     \chainin (m-2-1) [join={node[below,labeled] {\begin{smallmatrix} \text{splitting} \\ \text{+} \\ \text{List(Success)??} \end{smallmatrix}}}];
   }
   {
...
To effectively approximate the run-time behavior, we introduce a finite program \emph{contour}
\note{define}
@time@ that encodes the program execution history. The function @tick@ is used to refresh
the ``time'' and get a ''new time''. We use a list of expressions (which are drawn from the
control component) to encode the calling context history, and as we will see in Section~
...
the @step@ function in AAM is the lazy-list constructor, and here we turn it into an 
...
(Scala) through refunctionalization, then the call/return flow of the analyzed language
is naturally matched.

In fact, what we started with is an AAM with an unbounded stack which already precisely
...
To obtain a definitional abstract interpreter in direct style, 
