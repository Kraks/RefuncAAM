...
@ae@ to its caller which is contained in the top frame of the stack.
However, since we have added a meta-continuation into the state, an empty list of frames 
means we have reached the halt of \textit{one computation path}, and we should determine the
next state indicated by @NDCont@. Thus, we have an inner pattern matching on @ndk@:

\begin{itemize}
  \item If the closure set is an empty set (@Nil@ in the code since we use list to represent that set),
...
environment and a new store for the body expression of that closure.
We use the same frame list and time which are copied from the fork point rather
than current one.
The meta-continuation @new_ndk@ is also updated by the removal of that popped closure, which makes
  ...
  def continue(nds: NDState, seen: Set[State]): Set[State] = {
    ...
    konts match {
      ...
      case Frame(x, e, f_env)::konts =>
        ...
        val new_store = bstore.update(baddr, atomicEval(ae, env, bstore))
        drive_step(NDState(e, new_env, new_store, konts, new_time, ndk), seen) /* normal return */
    }
  }
\end{lstlisting}
...
\begin{lstlisting}
  def ndcontinue(nds: NDState, seen: Set[State]): Set[State] = {
    ...
    ndk match {
      case NDCont(Nil, _, _, _, _)::ndk =>
        /* transfer to the most recent fork point */
...
The nondeterministic operator @nd@ is also slightly rewritten to cache-passing style:
the continuation @k@ takes an additional argument of type @Cache@; the current accumulated
cache @acc.cache@ is exposed to @k@ when applied.
