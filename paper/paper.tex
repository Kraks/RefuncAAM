...
Since each case in function @step@ are statically known, we can inline the pattern match on the
...
and further prepare the interpreter for tranforming to direct-style later.
...
@konts@ and @ndk@ from the definition of state. Accordingly, the abstract interpreter
...
In the initial iteration, we first conservatively assume that all computations can be diverged, 
...
The @out@ cache of this iteration will be used as @in@ cache for the next iteration.
...
a @nd@ operator which is written with two layers of continuations. Further more, we added
