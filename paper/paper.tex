...
%% Title information
\title{Refunctionalization of Abstract Abstract Machines}         %% [Short Title] is optional;
                                        %% when present, will be used in
                                        %% header instead of Full Title.
%\titlenote{with title note}             %% \titlenote is optional;
                                        %% can be repeated if necessary;
                                        %% contents suppressed with 'anonymous'
\subtitle{Bridging the Gap between Abstract Abstract Machines and Abstract Definitional Interpreters (Functional Pearl)}        %% \subtitle is optional
...
\begin{abstract}
  ...
  abstract machines (AAMs) that perform abstract interpretation from abstract machines that
  perform concrete evaluation. Darais et al. apply the same underlying idea to monadic definitional interpreters,
  ...
  systematically transform small-step abstract abstract machines into big-step abstract
...
\keywords{refunctionalization, abstract machines, control-flow analysis, Scala}  %% \keywords are mandatory in final camera-ready submission
...
\begin{center}
 \begin{tikzpicture}
   ...
   }
   {
     [start chain, color=gray] \chainin (m-3-1);
     {
       ...
       [join={node[above, labeled] {\text{Functional correspondences for concrete semantic artifacts}}}];
     }
   }
 ...
 \end{tikzpicture}
 \caption{Functional correspondence between and transformations from AAMs to ADIs} \label{fig:trans}
\end{center}
\end{figure}
...
small-step AAMs and big-step ADIs by applying a series of well-known systematic transformations
...
\begin{itemize}
  ...
  \item In fact, we may also just use the same caching algorithm but with side effects such as
    ...
    interpreter with pushdown control-flow. \Citet{friedman-ai}, in a tutorial on abstract
...
defunctionalization is dual to Church encoding, and further case studies show that known
higher-order and first-order implementations can be related through defunctionalization
and refunctionalization, including backtracking algorithms (e.g., regular expression matcher),
...
\Citet{cousot1977abstract} discovered abstract interpretation as a sound approach to
approximate a program's run-time behavior. Control-flow analysis
is one instance of abstract interpretation on higher-order functional programs that can be traced to \citet{jones1981flow}. \Citet{Shivers:1988:CFA:53990.54007,Shivers:1991:SSC:115865.115884}
...
and several variants and improvements are proposed by \citet{vardoulakis-diss12} afterwards.
