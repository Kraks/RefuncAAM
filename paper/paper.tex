...
\begin{abstract}
  ...
  abstract machines (A\textsuperscript{2}Ms) that perform abstract interpretation from abstract machines that 
  ...
  and obtain monadic \emph{abstract} definitional interpreters (ADIs) that perform
  ...
  The refunctionalized A\textsuperscript{2}M is an abstract interpreter written in continuation-passing style (CPS)
  ...
  All transformations properly handle the collecting semantics and nondeterminism
...
\begin{center}
 ...
 \end{tikzpicture}
 \caption{Functional correspondence between and transformations from A\textsuperscript{2}Ms to ADIs} \label{fig:trans}
\end{center}
\end{figure}
...
small-step A\textsuperscript{2}Ms and big-step ADIs by applying a series of well-known systematic transformations
...
The @alloc@ function generates a fresh address, and always allocates an unique integer
...
The pushdown A\textsuperscript{2}M with an unbounded stack is also described by \citeauthor{van2012systematic} 
...
Unfortunately, even though other components in the states are finite, this A\textsuperscript{2}M with an unbounded
...
frames @f1@ and @f2@ onto the top of the current stack. However, no two existing stack
...
\begin{itemize}
  \item If the meta-continuation @ndk@ is also empty, then the machine has reached the end of \textit{all} 
    computations. Besides, the control component @ae@ now is an atomic value, therefore @None@ is returned.
  \item If the target closures set of the front-most @NDCont@ is empty (@Nil@ in the code),
    ...
    elements to its front, we resume to the most recent fork point by popping up the front-most 
...
then the linearized A\textsuperscript{2}M flattens the graph in depth-first order to a linear sequence of states.
...
Since each case in function @step@ is statically known, we can inline the pattern match on the
@Option@ type and replace it with returning @new_seen@ when @None@ is matched, or calling 
...
% first call to drive_step can be replaced with continue. ae is atomic, konts is empty.
...
and further prepare the interpreter for transforming to direct-style later.
...
continuation and meta-continuation:
...
Then to address the circularity of the meta-continuations, we define an auxiliary function
@makeMCont@ to construct @new_mcont@, which takes a list of closures as the argument and returns 
...
In this section, based on the refunctionalized A\textsuperscript{2}M, we further present several other 
...
\begin{itemize}
  \item The components of the configuration are lifted as arguments of the evaluation
    function.
  \item The refunctionalized A\textsuperscript{2}M collects all possible configurations, but our final target is an 
...
We will begin by discussing these changes separately, and finally see how they work together.
...
% why cache? problem of set of states.
...
We observe that in the refunctionalized A\textsuperscript{2}M, @makeMCont@ is invoked with the tail part of
...
Given a collection of elements of type @T@, an initial accumulator of type @Ans@,
a function @k@ that works on elements of type @T@, @nd@ chooses an element in
...
use the @nd@ operator to fill in the details. The first case of match is that @e@
is a @let@ expression:
...
value/stores @bdvss@. For each @VS@, the time-stamp and the store in @VS@ will be 
...
the evaluation function can be simplified to one layer continuation-passing style.
The use of two layers of continuations are unchained to two functions that each of them has one
continuation.
The only continuation of @aeval@ plays the same role as the one in a concrete CPS interpreter.
Furthermore, we added a cache along with the abstract interpreter and the @nd@ operator. 
...
In fact, what we started with is an unbounded-stack A\textsuperscript{2}M which already precisely
...
Back to direct-style on the case where @e@ appears as an atomic expression is straightforward: 
we simply unwrap the application of @continue@. Now the abstract evaluator @aeval@ looks as follows:
...
The case where @e@ is a @let@ expression is not that obvious. As previously identified, 
function @nd@ works on an unknown number of nondeterministic choices, and it takes a 
...
to the two applications of @nd@ in the refunctionalized form in the last section.
The function @choices@ also returns the latest cache to its left-hand side when invoked.
The cache will be still accumulated to the following calls of @choices@ or @aeval@, 
...
application of @aeval@ to set the delimiter.
...
The inverse transformation of the last step is basically CPS transforming the function @aeval@ and
