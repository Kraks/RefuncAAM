...
\begin{itemize}
  ...
  \item We simplify the continuations with a nondeterministic operator @nd@ such 
...
the collection and applies @k@ to it iteratively, and accumulates the value that
returned from @k@ when recuring on the rest of the collection. Eventually, @nd@
...
The use two layers of continuations are unchained to two functions that each of them has one
continuation.
Now the continuation of @aeval@ plays the same role as the one in a concrete CPS interpreter.
Furthermore, we added a cache along with the abstract interpreter and @nd@ operator. 
...
we simply remove the application of @continue@. Now the abstract evaluator @aeval@ looks as follows:
...
function @nd@ works on a unknown number of nondeterministic choices, and it takes a 
...
when the program is direct-style and pass it to @nd@.
...
it returns to its call-site where @nd@ will be invoked on the rest elements of the collection.
Now we can use this function to sequentialize the @let@ case.
...
For every nondeterministic choice in the abstract interpretation, we can simply call 
...
the delimited continuation @f@. We have two invocations of @choices@, which correspond 
to the two invocations of @nd@ in the refunctionalized form in the last section.
...
call of @aeval@ to set the delimiter.
