...
unexplored states are embedded into the meta-continuation.
...
First of all, since there is no first-order data types representing model of stacks,
...
As we are getting close to achieving abstract definitional interpreters, 
...
only takes a configuration and a set of explored configurations as arguments.
...
Before jumping into the details of refunctionalizing the continuations and constructing
...
Meanwhile, the @new_cont@ we defined above is used here, simply because different closures 
actually share the same continuation.
The case where the empty closures set occurs is also interesting. At this case,
the continuation simply applies its meta-continuation @m@, and the meta-continuation is the
outer meta-continuation @mcontinue@. It would be clear if we inductively think the base case 
where the set contains only one closure, then @makeMCont@ is invoked with an empty set, 
therefore the continuation should do nothing. %TODO: rewrite this part.
...
The initial continuation is invoked when one computation path reaches its end,
so we invoke the meta-continuation @m@ to continue exploring other paths.
The initial meta-continuation is invoked when all computation paths reach their ends,
...
In this section, based on the refunctionalized AAM, we present several other transformations 
toward a big-step abstract definitional interpreter:
...
\begin{itemize}
  ...
  \item Since our final target is an abstract definitional interpreter (though still
    ...
    returns a set of final values instead of collected configurations.
...
@aeval@. Given the cache and simplified one-layer continuation (as we will see later), 
...
for the current configuration and use them immediately (if present).
...
not, we update its values in the cache after evaluation. In terms of partial orders and 
lattices, it is the case that starts the Kleene iteration from the bottom of the lattice.\\

\textit{Nondeterminism Abstraction.}
We observe that in the refunctionalized AAM, @makeMCont@ actually repeats the prologue
part when matching a @let@ expression: they are both preparing a new environment and store 
for the body expression of a closure. %TODO motivation
We present a nondeterministic operator that abstracts this pattern:
...
to continuation-passing style. Besides, these continuations have to be manifestly embedded
